...
$requeteCount = "SELECT COUNT(*) AS countPolice FROM polices_assurance
                 WHERE numero_assurance LIKE '%$numeroAssurance%'";
...
$nbrPolice = $tabCount['countPolice'];
$reste = $nbrPolice % $size;
$nbrPage = ($reste === 0) ? ($nbrPolice / $size) : (floor($nbrPolice / $size) + 1);
?>
<!DOCTYPE HTML>
<HTML>
<head>
    ...
    <div class="container">
        <div class="panel panel-success margetop60">
            ...
            <div class="panel-body">
                <form method="get" action="polices_assurance.php" class="form-inline">
                    ...
                    <tbody>
                        <?php while ($police = $resultatPolice->fetch()) { ?>
                            <tr>
                                ...
                                <?php if ($_SESSION['user']['role'] == 'ADMIN') { ?>
                                    <td>
                                        <a href="editerPolice.php?id_police=<?php echo htmlspecialchars($police['id_police']); ?>">
                                            <span class="glyphicon glyphicon-edit"></span>
                                        </a>
                                        &nbsp;
                                        <a onclick="return confirm('Êtes-vous sûr de vouloir supprimer cette police ?')"
                                            href="supprimerPolice.php?id_police=<?php echo htmlspecialchars($police['id_police']); ?>">
                    ...
                    <ul class="pagination">
                        <?php for ($i = 1; $i <= $nbrPage; $i++) { ?>
                            <li class="<?php if ($i == $page) echo 'active'; ?>">
                                <a href="polices_assurance.php?page=<?php echo $i; ?>&numeroAssurance=<?php echo htmlspecialchars($numeroAssurance, ENT_QUOTES); ?>">
                                    <?php echo $i; ?>
                                </a>
                            </li>
                        <?php } ?>
